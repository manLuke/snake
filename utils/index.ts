...
export const isCollisionWithItself = (coordinate: Coordinate, snake: Snake) => {
  ...
  const isInsideSnake = snake.some(
    (snakeCoordinate) => snakeCoordinate.x === coordinate.x && snakeCoordinate.y === coordinate.y
  );
...
export const getFirstValidDirection = (directions: number[]): number => {
  for (let i = 0; i < directions.length; i++) {
    if (!isGoingBack(directions[i], directions[directions.length - 1])) {
      return directions[i];
    }
  }
...
export const moveSnake = (snake: Snake, food: Coordinate, directionArr: number[]): NewSnakePosition => {
  const eaten = isEating(snake, food);
  const newPosition = {
    x: snake[0].x,
    y: snake[0].y,
  };
  const direction = getFirstValidDirection(directionArr);
